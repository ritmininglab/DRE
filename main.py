...
from losses import CustomizedLoss
...
def main():
    ...
    if args.lamb:
        args.name = args.name+'_'+str(args.lamb)
...
def main_worker(args):
    args.gpu = None
    train, validate, modifier, get_output = get_trainer(args)
    ...
    print("Total Trainable Parameters", total_param)
    if args.pretrained:
    ...
    if args.evaluate:
        ...
        if (args.set =='CIFAR10' or args.set=='CIFAR100') and args.noise_type:
            if args.noise_type=='SVHN':
                 val_loader = getattr(datas, 'SVHN')(args).val_loader
            else:
                val_loader = getattr(datas, args.set+'C')(args).val_loader
                
            identifier = args.arch+'_'+args.set+'_'+net_type+'_'+args.noise_type
            if args.lamb:
                identifier+='_'+str(args.lamb)
            output_path = 'outputs/output_'+identifier
            gt_path = 'outputs/gts_'+identifier
            loss_path = 'outputs/losses_'+identifier
            ...
            identifier = args.arch+'_'+args.set+'_'+net_type
            if args.lamb:
                identifier+='_'+str(args.lamb)
            output_path = "outputs/output_"+identifier
            gt_path = "outputs/gts_"+identifier
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1, acc5, test_loss = validate(data.val_loader, model, criterion, args, None, epoch)
    ...
    train_acc1s = np.array(train_acc1s)
    test_acc1s = np.array(test_acc1s)
    np.save(os.path.join(run_base_dir, 'train_losses.npy'), train_losses)
    np.save(os.path.join(run_base_dir, 'train_acc1.npy'), train_acc1s)
    np.save(os.path.join(run_base_dir, 'test_losses.npy'), test_losses)
    np.save(os.path.join(run_base_dir, 'test_acc1.npy'), test_acc1s)
    write_result_to_csv(
        best_acc1=best_acc1,
        best_acc5=best_acc5,
        best_train_acc1=best_train_acc1,
...
def get_trainer(args):
    ...
    return trainer.train, trainer.validate, trainer.modifier, trainer.get_output
